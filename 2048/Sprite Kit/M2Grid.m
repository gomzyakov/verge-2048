    ...
    if (self = [super init]) {
        ...
        for (NSInteger i = 0; i < dimension; i++) {
            ...
            for (NSInteger j = 0; j < dimension; j++) {
                [array addObject:[[Cell alloc] initWithPosition:M2PositionMake(i, j)]];
            }
...
- (Cell *)cellAtPosition:(M2Position)position
{
...
- (GeometricTile *)tileAtPosition:(M2Position)position
{
    Cell *cell = [self cellAtPosition:position];
...
{
    Cell *cell = [self randomAvailableCell];
    if (cell) {
        GeometricTile *tile = [GeometricTile insertNewTileToCell:cell];
...
{
    [self forEach:^(M2Position position) {
         GeometricTile *tile = [self tileAtPosition:position];
